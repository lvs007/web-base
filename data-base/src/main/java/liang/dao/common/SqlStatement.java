...
public class SqlStatement {
     ...
     * SQL语句占位符?对应的参数值
     */
    private final List<Object> preparedParameters = new ArrayList<Object>();
    /**
     * 解析后含占位符的SQL语句
     */
    ...
    public Object[] preparedParameters() {
        return preparedParameters.toArray(new Object[0]);
    }
     ...
     * @param parameters
     */
    private static void parseSql(String sql, Map<String, Object> parameters, StringBuilder originalSql, StringBuilder preparedSql, List<Object> preparedParameters) {
        //这里统计参数个数
        int totalCount = 0;
        int lastIndex = 0;
        String sqlToUse = sql;
     ...
     */
    private static void substituteNamedParameter(StringBuilder sqlBuilder, Object paramValue, List<Object> preparedParameters) {
        if (paramValue == null) {
            sqlBuilder.append("?");
            preparedParameters.add(paramValue);
        } else if (paramValue instanceof Collection) {
            substituteNamedParameter(sqlBuilder, ((Collection) paramValue).toArray(new Object[0]), preparedParameters);
            ...
            for (int i = 0; i < length; i++) {
                ...
                substituteNamedParameter(sqlBuilder, Array.get(paramValue, i), preparedParameters);
            }
            sqlBuilder.append(")");
        } else {
            sqlBuilder.append("?");

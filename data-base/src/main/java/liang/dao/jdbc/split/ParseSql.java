...
import liang.dao.jdbc.annotation.DataSource;
...
import liang.dao.jdbc.split.db.DbConfig;
import liang.dao.jdbc.split.listener.FailListener;
import liang.dao.jdbc.split.parse.SqlParseManager;
...
public abstract class ParseSql {
    ...
    public List<Sql> parseSql(Sql sql) {
        ...
        sql.setDml(dml);
        SqlParseManager.findSqlParse(dml).setParseSql(this).setSplitConfig(splitConfig).parse(sql, sqlStr, table, tableRule, sqlList);
        stopWatch.stop();

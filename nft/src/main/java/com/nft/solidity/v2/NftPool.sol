...
contract IERC721 {
    ...
    function create(address owner, uint8 level) public returns (uint48);
    function nftdesc(uint48 nftId) public view returns (address owner, uint8 level);
...
contract LPTokenWrapper {
    ...
    mapping(address => uint48) private _nfts;
    ...
    function _stake(uint256 amount, uint48 nftId) internal {
        require(_balances[msg.sender] <= 0, "you have already stake, you can't double stake");
        (address owner, uint8 level) = nft.nftdesc(nftId);
        uint256 rate = 0;
        if(level == 1){
            rate = 10;
        }else if(level == 2){
        ...
        uint256 amountCount = amount.add(amount.mul(rate).div(100));
        _totalSupply = _totalSupply.add(amountCount);
        _balances[msg.sender] = _balances[msg.sender].add(amountCount);
        _nfts[msg.sender] = nftId;
    ...
    function withdraw() internal {
        require(_balances[msg.sender] > 0, "not balance");
        uint48 nftId = _nfts[msg.sender];
        (address owner, uint8 level) = nft.nftdesc(nftId);
        uint256 rate = 0;
        if(level == 1){
            rate = 10;
        ...
        uint256 amountSource = _balances[msg.sender].mul(100).div(100+rate);
        _totalSupply = _totalSupply.sub(_balances[msg.sender]);
        _balances[msg.sender] = 0;
        tokenAddr.safeTransfer(msg.sender, amountSource);
        nft.transfer(msg.sender, nftId);
    }
}

contract MHPool is LPTokenWrapper, IRewardDistributionRecipient {
    //usdt:TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t

    ITRC20 public xgToken;
    uint256 public constant DURATION = 2_419_200; //28  days

    uint256 public starttime = 1616673600; //2021-03-25 20:00 (HKT)
    ...
    event Staked(address indexed user, uint256 amount, uint256 nftId);
    ...
    constructor(address lp,address _token, uint256 _starttime, address _nft) public{
        tokenAddr = ITRC20(lp);
        xgToken = ITRC20(_token);
    ...
    function stake(uint256 amount, uint48 nftId) public updateReward(msg.sender) checkStart {
        require(amount > 0, "Cannot stake 0");
        require(nft.ownerOf(nftId) == msg.sender, "this nftId are not your");
        _stake(amount, nftId);
        emit Staked(msg.sender, amount, nftId);
    }

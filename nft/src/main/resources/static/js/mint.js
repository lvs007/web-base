...
async function withdraw(type,amount)
{
    ...
    try{
      ...
      checkTransaction(res,"Withdraw Success.");
    }catch (error) {
      console.log(error);
      alert('Withdraw Fail!');
    }
...
async function harvest(type)
{
    ...
    try{
      ...
      checkTransaction(res,"Harvest Success.");
...
async function exit(type)
{
    ...
    try{
      ...
      checkTransaction(res,"Exit Success.");
    }catch (error) {
      console.log(error);
    }
}
...
async function stakeDay(type,amount) {
  ...
  try{
    ...
    checkTransaction(res,"Stake Success.");
  }catch (error) {
    console.log(error);
  }
}
...
async function buyBox(){
  ...
  try{
    await approve(mhTokenContractAdd,mhboxContractAdd,boxPrice);
...
async function buyAndOpenBox(){
  ...
  try{
    ...
    checkTransaction(res,"Buy And Open Success.");
    mhnftlist();
  } catch (error) {
    console.log(error);
    alert('Buy fail');
...
async function openBox(boxId){
  ...
  try{
    ...
    checkTransaction(res,"Open Success.");
    boxlist();
  } catch (error) {
    console.log(error);
    alert('Open fail');
...
async function mergeNft(){
  ...
  try{
    ...
    checkTransaction(res,"Merge Nft Success.");
    mhnftlist();
  } catch (error) {
    console.log(error);
...
async function approveNft(contractAddress,to,nftId){
  try {
      var issuerAddress = window.tronWeb.defaultAddress.hex;
      var parameter1 = [{type:'address',value:''},{type:'uint48',value:'0'}]
      parameter1[0].value = tronWeb.address.fromHex(to);
      parameter1[1].value = nftId;

      var tx = await tronWeb.transactionBuilder.triggerSmartContract(contractAddress,"approve(address,uint48)", {},parameter1,issuerAddress);
      var signedTx = await tronWeb.trx.sign(tx.transaction);
      var broastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    } catch (error) {
        console.log(error);

...
contract ERC721 is ERC165, IERC721 {
    ...
    function _nftBo2uint(NftBo memory nftbo) internal pure returns (uint) {
        uint n = uint(nftbo.owner);
        n = (n<<48) | nftbo.index;
        return n;
    }
    // extract an NftBo from a 256b integer
    function _uint2NftBo(uint n) internal pure returns (NftBo memory) {
        NftBo memory nftbo;
        nftbo.index = uint32(n & ((1<<48)-1));
        n = n >> 48;
        nftbo.owner = address(n);
        return order;
    }
    ...
    function _mint(address owner, string memory url, string memory title) internal returns (uint256){
        ...
        nftdesc[nftid] = nftBo;
        nftFinalData[nftid] = NftBoFinal(uint48(now), nftid, title, url);
...
contract ERC721Enumerable is ERC165, ERC721, IERC721Enumerable {
    ...
    function tokensOfOwnerByIndexs(address owner, uint256[] memory indexs) public view returns (NftBoFinal[] memory nftBos) {
        for (uint128 i = 0; i < indexs.length; i++) {
            uint128 id = balanceOf_[owner][indexs[i]];
            nftBos[i] = nftFinalData[id];
        }
        return nftBos;
    }

    function tokensByIndexs(uint128[] memory tokenIds) public view returns (NftBoFinal[] memory nftBos) {
...
contract Market is ERC721Full, Ownable {
    ...
    function getselldesc(uint128 tokenId) public view returns (NftInfo memory nftinfo,
    ...
    function querypm(uint256[] memory indexs, uint256 select) private view returns (NftPM[] memory nftpms, NftBoFinal[] memory nftbos){
        uint128[] memory ids = new uint128[](indexs.length);
    ...
    function getpmdesc(uint128 tokenId) public view returns (NftPM memory nftpmEntity, NftBoFinal memory nftboEntity){
        require(pming[nftpm[tokenId].index] == tokenId && nftpm[tokenId].state == 0, "This nft is not for auction");
